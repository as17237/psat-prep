...
- **Student's interrupted exam (2026-08-29) — CLOSED 2026-08-30, loss accepted by owner.** The resume banner never appeared on the student's device, and the owner decided to write the exam off rather than chase it: *"Let's just keep that exam out. Ignore. We will accept the loss there."* Scope of the loss is only that exam's **score report and test-history entry** — the per-question answers were written to `psat_progress`/`psat_srs` *before* the crash point, so they still feed analytics, mistakes tracking and SRS scheduling. Cloud history therefore stays at 9 exams; that is expected, not a symptom. A stale `psat_active_exam_state` may still sit in that browser; it blocks nothing (starting a new exam overwrites it), and clicking Discard on any resume banner is safe. **Do not reopen this.** Original detail:
- (historical) **Student's interrupted exam (2026-08-29):** an exam crashed at Finish under the pre-hotfix code. Answers are safe in their browser's localStorage; the exam records only when the student clicks **Resume → Finish** in the fixed app. Until then cloud history stays at 9 exams. Check: `curl -s '<api>/api/sync?student_name=default_student'` → examHistory length.
