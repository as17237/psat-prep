...
- **⚠️ OPEN ISSUE (mobile):** `analytics.spec.js:62` "Data & Sync … Reset is guarded by a confirm" fails ONLY at the 390px mobile viewport — the Reset button is `toBeVisible` but `locator.click` times out (pointer intercepted). Passes on desktop; `scrollIntoViewIfNeeded` did NOT fix it, so something overlays the button at mobile (candidate: the taller sticky header after the nav-strip relocation, or a mobile-layout overlap in the Data & Sync `.card`). **Needs a browser screenshot at 390px to diagnose** — do this with the app open, not blind. Not a desktop/usability blocker; the student uses desktop.
- **Lighthouse:** still pending (needs a real Chrome; couldn't run headless here).
- **WI-13 remaining to fully close:** fix the mobile Reset-click issue → re-run mobile green → Lighthouse ≥90 → merge to `main` (preflight-gated; `deploy_common.sh` was touched).
